-- ╔══════════════════════════════════════════════════════╗
-- ║  ROSE — Neovim Colorscheme                          ║
-- ╚══════════════════════════════════════════════════════╝
--
-- Desaturated pastel-pink dark colorscheme. Primary accent is dusty
-- rose; cool teal as the secondary. Deep blacks, soft rose-white text.
-- Pairs with the desktop ROSE theme.
...
function M.setup()
	...
	local c = {
		...
		fg        = "#E2D6DB",
		fg_dim    = "#A08892",
		fg_dark   = "#7A6770",       -- line numbers / dim chrome (raised for legibility)
		comment   = "#9A8690",       -- comments — WCAG-AA readable on dark
		cream     = "#D8CCD1",
		gold      = "#CBA1B2",       -- PRIMARY rose accent (keeps name for compat)
		red       = "#C76B7A",
		red_light = "#E08A98",
		green     = "#7FB394",       -- sage — cool, clearly not pink
		green_l   = "#9FCFAD",       -- bright sage for strings
		yellow    = "#D4B58C",       -- warm rose-gold
		blue      = "#7C9CC0",
		blue_l    = "#9BBEE4",
		purple    = "#C18FB0",       -- rose-magenta
		purple_l  = "#D9AECC",
		cyan      = "#7FA8AD",       -- desktop secondary teal
		cyan_l    = "#9CC6CB",
		border    = "#4a4250",       -- rose-grey for window separation
		selection = "#2a2026",
		visual    = "#3a2e36",       -- rose-tinted selection
		none      = "NONE",
	}
	...
	hl("DiffAdd",        { bg = "#16241c" })
	hl("DiffChange",     { bg = "#161e28" })
	hl("DiffDelete",     { bg = "#2a1620" })
	hl("DiffText",       { bg = "#2e2a34" })
	...
	-- ── Indent Blankline ──
	hl("IblIndent",   { fg = "#332a35" })  -- faint rose-grey indent hearts
	hl("IblScope",    { fg = c.gold })
	...
	-- ── Background + dashboard accents ──
	-- SOLID opaque editor bg for long-session readability. kitty's
	-- background_opacity only dims default-bg cells, so an explicit bg here
	-- stays crisp/opaque (the shell stays translucent, code does not).
	-- To go transparent again (pink wallpaper through code): set EDITOR_BG = c.none.
	local EDITOR_BG = "#0d0a0e"
	hl("Normal",      { fg = c.fg,      bg = EDITOR_BG })
	hl("NormalNC",    { fg = c.cream,   bg = EDITOR_BG })
	hl("NormalFloat", { fg = c.fg,      bg = c.bg1 })
	hl("SignColumn",  { fg = c.fg_dark, bg = EDITOR_BG })
	hl("VertSplit",   { fg = c.border,  bg = EDITOR_BG })
	hl("LineNr",      { fg = c.fg_dark, bg = EDITOR_BG })
	hl("CursorLine",  { bg = "#161018" })
	hl("BabyBlue",    { fg = c.gold })    -- dashboard header
	hl("PastelPink",  { fg = c.purple })  -- dashboard footer — actually pink now
	hl("Lilac",       { fg = c.purple })
	hl("AlphaHeart",  { fg = "#DDB6C7", bold = true })  -- spinning dashboard heart
end
